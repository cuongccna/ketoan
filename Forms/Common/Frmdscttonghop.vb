...
Partial Friend Class FrmDsCTTonghop
    ...
    'End Sub
    Private Sub ChkLoai_CheckStateChanged(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles _ChkLoai_12.CheckStateChanged, _ChkLoai_11.CheckStateChanged, _ChkLoai_0.CheckStateChanged, _ChkLoai_1.CheckStateChanged, _ChkLoai_2.CheckStateChanged, _ChkLoai_3.CheckStateChanged, _ChkLoai_9.CheckStateChanged, _ChkLoai_10.CheckStateChanged, _ChkLoai_8.CheckStateChanged, _ChkLoai_7.CheckStateChanged, _ChkLoai_13.CheckStateChanged, _ChkLoai_14.CheckStateChanged
        If _ChkLoai_7.CheckState = CheckState.Checked Then
            For i As Integer = 0 To 3
                ChkLoai(i).CheckState = CheckState.Unchecked
                ChkLoai(i).Enabled = False
            Next
        ...
        Else
            ...
            _ChkLoai_0.Enabled = True
            _ChkLoai_1.Enabled = True
            _ChkLoai_2.Enabled = True
            _ChkLoai_3.Enabled = True

            _ChkLoai_8.Enabled = True
            _ChkLoai_9.Enabled = True
            _ChkLoai_10.Enabled = True
            _ChkLoai_11.Enabled = True
            _ChkLoai_12.Enabled = True
            _ChkLoai_13.Enabled = True



        End If
        _Command_5.Enabled = (_ChkLoai_7.CheckState = CheckState.Unchecked)
        _Command_7.Visible = (_ChkLoai_1.CheckState = CheckState.Checked) Or (_ChkLoai_2.CheckState = CheckState.Checked)
        Dim vs As Boolean = (_ChkLoai_1.CheckState = CheckState.Checked Or _ChkLoai_2.CheckState = CheckState.Checked Or _ChkLoai_8.CheckState = CheckState.Checked)
        _Label_8.Visible = vs
        _Label_9.Visible = vs
        _CboN_0.Visible = vs
        _CboN_1.Visible = vs
    ...
    Private Sub Command_ClickEvent(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles _Command_7.Click, _Command_4.Click, _Command_3.Click, _Command_2.Click, _Command_1.Click, _Command_0.Click, _Command_6.Click, _Command_5.Click
        ...
        Select Case Index
            Case 0, 7
                If ChkTaikhoan(0).CheckState = CheckState.Checked AndAlso ConvertToDblSafe((txtShTk(0).Tag)) = 0 Then
            ...
            Case 3, 4
                DSCTu(Index - 3)
            Case 5
                Dim f2 As New FrmChungtutonghop()
                f2.ShowDialog()
                With GrdChungtu
                    .Col = 5
                    For Each sItem As ListViewEx.GLItem In .SelectedItems
                        ...
                        If ms > 0 Then
                            pPhieu = 0
                            f2.VaoSoNK(ConvertToDblSafe(GrdChungtu.CtlText))
                        End If
                    Next
                End With
            ...
            Case 6
                ...
                If dk1 <> 1 Then
                    MessageBox.Show(Ngonngu("Tài khoản có chi tiết hoặc không có trong hệ thống tài khoản ! ", "Account details or not in the account list !"), My.Application.Info.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Exit Sub
                End If
...
Xong:
    ...
    Private Sub DoisoChungtu(ByRef shtk As String, ByRef mvt As Integer, ByRef mts As Integer, ByRef mcn As Integer, ByRef shd As String)
        ...
        If OptLK(0).Checked Then
            sql = String.Format("{0}{1}{2} GROUP BY MaCT", _
                      sql, _
                      WThang("ThangCT", CboThang(0).SelectedIndex, CboThang(1).SelectedIndex), _
        ...
        If ChkLoai(13).CheckState = CheckState.Checked Then
            sql = String.Format("SELECT DISTINCT ChungTu{0}.MaCT, ChungTu{0}.SoHieu, NgayCT, NgayGS, ChungTu{0}.DienGiai{1} AS DG,TPS,[User_ID],ChungTu{0}.MaLoai,chungtu{0}.manhapxuat,chungtu{0}.thangct,chungtu{0}.maso FROM ((((ChungTu{0} INNER JOIN (" & sql & ") as MienTru ON ChungTu{0}.MaCT=MienTru.MaCT) LEFT JOIN HoaDon{0} ON ChungTu{0}.MaSo=HoaDon{0}.MaSo) LEFT JOIN CTCongcudungcu ON ChungTu{0}.MaCT = CTCongcudungcu.MaCTKT) LEFT JOIN HeThongTK ON ChungTu{0}.MaTKNo = HeThongTK.MaSo) LEFT JOIN HeThongTK AS HeThongTK_1 ON ChungTu{0}.MaTKCo = HeThongTK_1.MaSo WHERE ChungTu{0}.MaLoai=1 OR ChungTu{0}.MaLoai=2 ", _
                      sh, _
                      (IIf(pNN = 1, "E", String.Empty)))
            sql1 = String.Format("SELECT DISTINCT ChungTu{0}.MaCT, ChungTu{0}.SoHieu, NgayCT, NgayGS, ChungTu{0}.DienGiai{1} AS DG,TPS,[User_ID],ChungTu{0}.MaLoai,chungtu{0}.manhapxuat FROM ((((ChungTu{0} INNER JOIN (" & sql & ") as MienTru ON ChungTu{0}.MaCT=MienTru.MaCT) LEFT JOIN HoaDon{0} ON ChungTu{0}.MaSo=HoaDon{0}.MaSo) LEFT JOIN CTCongcudungcu ON ChungTu{0}.MaCT = CTCongcudungcu.MaCTKT) LEFT JOIN HeThongTK ON ChungTu{0}.MaTKNo = HeThongTK.MaSo) LEFT JOIN HeThongTK AS HeThongTK_1 ON ChungTu{0}.MaTKCo = HeThongTK_1.MaSo WHERE (TRUE)", _
                       sh, _
                       (IIf(pNN = 1, "E", String.Empty)))
        Else
            sql = String.Format("SELECT DISTINCT ChungTu{0}.MaCT, ChungTu{0}.SoHieu, NgayCT, NgayGS, ChungTu{0}.DienGiai{1} AS DG,TPS,[User_ID],ChungTu{0}.MaLoai,chungtu{0}.manhapxuat,chungtu{0}.thangct,chungtu{0}.maso FROM ((((ChungTu{0} INNER JOIN (" & sql & ") as MienTru ON ChungTu{0}.MaCT=MienTru.MaCT) LEFT JOIN HoaDon{0} ON ChungTu{0}.MaSo=HoaDon{0}.MaSo) LEFT JOIN CTTaiSan ON ChungTu{0}.MaCT = CTTaiSan.MaCTKT) LEFT JOIN HeThongTK ON ChungTu{0}.MaTKNo = HeThongTK.MaSo) LEFT JOIN HeThongTK AS HeThongTK_1 ON ChungTu{0}.MaTKCo = HeThongTK_1.MaSo WHERE ChungTu{0}.MaLoai=1 OR ChungTu{0}.MaLoai=2 ", _
                      sh, _
                      (IIf(pNN = 1, "E", String.Empty)))
            sql1 = String.Format("SELECT DISTINCT ChungTu{0}.MaCT, ChungTu{0}.SoHieu, NgayCT, NgayGS, ChungTu{0}.DienGiai{1} AS DG,TPS,[User_ID],ChungTu{0}.MaLoai,chungtu{0}.manhapxuat FROM ((((ChungTu{0} INNER JOIN (" & sql & ") as MienTru ON ChungTu{0}.MaCT=MienTru.MaCT) LEFT JOIN HoaDon{0} ON ChungTu{0}.MaSo=HoaDon{0}.MaSo) LEFT JOIN CTTaiSan ON ChungTu{0}.MaCT = CTTaiSan.MaCTKT) LEFT JOIN HeThongTK ON ChungTu{0}.MaTKNo = HeThongTK.MaSo) LEFT JOIN HeThongTK AS HeThongTK_1 ON ChungTu{0}.MaTKCo = HeThongTK_1.MaSo WHERE (TRUE)", _
                       sh, _
                       (IIf(pNN = 1, "E", String.Empty)))
        End If
        If shd.Length > 0 Then
            sql = String.Format("{0} AND (HoaDon{1}.SoHD LIKE '{2}%' OR ChungTu{1}.SoHieu LIKE '{2}%')", sql, sh, shd)
            sql1 = String.Format("{0} AND (HoaDon{1}.SoHD LIKE '{2}%' OR ChungTu{1}.SoHieu LIKE '{2}%')", sql1, sh, shd)
        End If
        If mvt > 0 Then
            ...
            sql1 = String.Format("{0} AND (ChungTu{1}.MaLoai = 1 OR ChungTu{1}.MaLoai = 2 OR ChungTu{1}.MaLoai = 8 OR ChungTu{1}.MaLoai = 13 OR ChungTu{1}.MaLoai = 14) AND (MaVattu = {2})", sql1, sh, ConvertToStrSafe(mvt))
        End If
        If mts > 0 Then
            sql = String.Format("{0} AND (CTTaiSan.MaTS = {1})", sql, ConvertToStrSafe(mts))
            sql1 = String.Format("{0} AND (CTTaiSan.MaTS = {1})", sql1, ConvertToStrSafe(mts))
        End If
        If mcn > 0 Then
            sql = String.Format("{0} AND (MaKH = {1} OR MaKhachHang = {1} OR MaKHC={1})", sql, ConvertToStrSafe(mcn))
            sql1 = String.Format("{0} AND (MaKH = {1} OR MaKhachHang = {1} OR MaKHC={1})", sql1, ConvertToStrSafe(mcn))
        End If
        If shtk.Length > 0 Then
            ...
            If ChkTaikhoan(7).CheckState = CheckState.Checked Then
                ...
                sql1 = String.Format("{0} AND ((HethongTK.TK_ID={1} OR HethongTK_1.TK_ID={2}) AND CT_ID=0)", _
                           sql1, _
                           ConvertToStrSafe(TKCNPT_ID), _
                           ConvertToStrSafe(TKCNKH_ID))
            End If
        End If
        If ChkTaikhoan(4).CheckState = CheckState.Checked And CboN(2).SelectedIndex >= 0 Then
            ...
            sql1 = String.Format("{0} AND ChungTu{1}.CTGS={2}", sql1, sh, ConvertToStrSafe(CboN(2).SelectedValue))
        End If
        If ChkTaikhoan(8).CheckState = CheckState.Checked And ConvertToDblSafe((txtShTk(6).Tag)) > 0 Then
            ...
            sql1 = String.Format("{0} AND ChungTu{1}.MaTP={2}", sql1, sh, ConvertToStrSafe(txtShTk(6).Tag))
        End If
        If ChkTaikhoan(6).CheckState = CheckState.Checked Then
            sql = String.Format("{0} AND (ChungTu{1}.User_ID={2})", sql, sh, ConvertToStrSafe(CboN(3).SelectedValue))
            sql1 = String.Format("{0} AND (ChungTu{1}.User_ID={2})", sql1, sh, ConvertToStrSafe(CboN(3).SelectedValue))
        End If
        ...
        If Not (loaict.Length = 0) Then
            If loaict.Length > 0 Then
                sql = String.Format("{0} AND ({1})", sql, Strings.Left(loaict, loaict.Length - 4))
                sql1 = String.Format("{0} AND ({1})", sql1, Strings.Left(loaict, loaict.Length - 4))
            End If
            If (ChkLoai(13).CheckState = CheckState.Checked Or ChkLoai(1).CheckState = CheckState.Checked Or ChkLoai(2).CheckState = CheckState.Checked Or ChkLoai(8).CheckState = CheckState.Checked) And CboN(0).SelectedIndex > 0 Then
                sql = String.Format("{0} AND (ChungTu{1}.MaKho={2})", sql, sh, ConvertToStrSafe(CboN(0).SelectedValue))
                sql1 = String.Format("{0} AND (ChungTu{1}.MaKho={2})", sql1, sh, ConvertToStrSafe(CboN(0).SelectedValue))
            End If
            If (ChkLoai(13).CheckState = CheckState.Checked Or ChkLoai(1).CheckState = CheckState.Checked Or ChkLoai(2).CheckState = CheckState.Checked Or ChkLoai(8).CheckState = CheckState.Checked) And CboN(1).SelectedIndex > 0 Then
                sql = String.Format("{0} AND (ChungTu{1}.MaNguon={2})", sql, sh, ConvertToStrSafe(CboN(1).SelectedValue))
                sql1 = String.Format("{0} AND (ChungTu{1}.MaNguon={2})", sql1, sh, ConvertToStrSafe(CboN(1).SelectedValue))
            End If
            If pProcessMode = 1 Then sql = String.Format("{0} AND XuLy<2 ", sql)
            sql = String.Format("{0} GROUP BY ChungTu{1}.MaCT,[User_ID],ChungTu{1}.MaLoai,ChungTu{1}.SoHieu, ChungTu{1}.NgayCT, ChungTu{1}.NgayGS, ChungTu{1}.DienGiai{2},TPS,chungtu{1}.manhapxuat,chungtu{1}.thangct,chungtu{1}.maso ", sql, sh, (IIf(pNN = 1, "E", String.Empty)))
            sql1 = String.Format("{0} GROUP BY ChungTu{1}.MaCT,[User_ID],ChungTu{1}.MaLoai,ChungTu{1}.SoHieu, ChungTu{1}.NgayCT, ChungTu{1}.NgayGS, ChungTu{1}.DienGiai{2},TPS,chungtu{1}.manhapxuat ", sql1, sh, (IIf(pNN = 1, "E", String.Empty)))
            Select Case ord
                Case 0
            ...
            Do While rs_chungtuIndex < rs_chungtu.Rows.Count
                ...
                End If
                ExecSQLNonQuery(String.Format("UPDATE chungtu set manhapxuat=N'{0}' WHERE maso={1}", _
                                    sochungtudoi, _
                                    rs_chungtuItem("MaSo")))
            ...
            Do While rs_chungtuIndex < rs_chungtu.Rows.Count
                ...
                If GrdChungtu.Rows < MaxGridRow Then
                    ...
                    GrdChungtu.AddItem(String.Format("{0}{1}{2}{1}{3}{1}{4}{1}{5}{1}{6}", _
                                           ConvertToStrSafe(rs_chungtuItem("manhapxuat")), _
                                           Chr(9), _
                                           Format(rs_chungtuItem("NgayCT"), Mask_D), _
                                           Format(rs_chungtuItem("NgayGS"), Mask_D), _
    ...
    Private Sub CmdTK_ClickEvent(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles _CmdTK_2.Click, _CmdTK_3.Click, _CmdTK_1.Click, _CmdTK_0.Click, _CmdTK_6.Click
        ...
        Select Case Index
            ...
            Case 2
                txtShTk(2).Text = frmDSTaiSan.ChonTaiSan(txtShTk(2).Text, CboThang(0).SelectedIndex, CboThang(1).SelectedIndex)
            Case 3
    ...
    ''' <param name="eventArgs"></param>
    ''' <remarks></remarks>
    Private Sub FrmDsCTTonghop_KeyDown(ByVal eventSender As Object, ByVal eventArgs As KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Sub FrmDsCTTonghop_Load(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Load
        Try
            'set icon
            ...
            If pPhieu > 0 Then Me.Text = "Danh sách các phiếu chưa vào sổ"
            ...
            If chiso0 = -1 Then AddMonthToCbo(CboThang(0))
            If chiso1 = -1 Then AddMonthToCbo(CboThang(1))
            ...
            Dim MienTru As String = String.Format("SELECT MaSo As F2,SoHieu + ' - ' + DienGiai As F1 FROM NguonNhapXuat  UNION SELECT 0, N'{0}' FROM License", (" Toàn bộ"))
            Int_RecsetToCbo(String.Format("SELECT * FROM ({0}) AS MienTru ORDER BY F1", MienTru), CboN(1))
            MienTru = String.Format("SELECT MaSo As F2,TenKho As F1 FROM KhoHang   UNION SELECT 0, N'{0}' FROM License", (" Toàn bộ"))
    ...
    Private Sub GrdChungtu_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles GrdChungtu.KeyUp
    ...
    Private Sub txtShTk_Leave(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles _txtShTk_5.Leave, _txtShTk_0.Leave, _txtShTk_2.Leave, _txtShTk_4.Leave, _txtShTk_6.Leave, _txtShTk_1.Leave, _txtShTk_3.Leave
        ...
        Select Case Index
            Case 0
                ...
                Command(6).Visible = ChkTaikhoan(0).CheckState = CheckState.Checked And Strings.Left(txtShTk(0).Text, 1) <> "." And Strings.Len(txtShTk(0).Text) <> 0
                If Strings.Left(txtShTk(0).Text, 4) = "1422" Then
                    ChkTaikhoan(8).Text = "Hợp đồng phát sinh"
                Else
                    ChkTaikhoan(8).Text = "Công trình, sản phẩm"
                End If
            Case 1
    ...
    Private Sub LietKeChungtu(ByRef shtk As String, ByRef mvt As Integer, ByRef mts As Integer, ByRef mcn As Integer, ByRef shd As String)
        ...
        If OptLK(0).Checked Then
            sql = String.Format("{0}{1}{2} GROUP BY MaCT", sql, WThang("ThangCT", CboThang(0).SelectedValue, CboThang(1).SelectedValue), (IIf(pProcessMode = 1, " AND XuLy<2", String.Empty)))
        Else
        ...
        If ChkLoai(13).CheckState = CheckState.Checked Then
            sql = String.Format("SELECT DISTINCT ChungTu{0}.MaCT, ChungTu{0}.SoHieu, NgayCT, NgayGS, ChungTu{0}.DienGiai{1} AS DG,TPS,[User_ID],ChungTu{0}.MaLoai FROM ((((ChungTu{0} INNER JOIN (" & mientru & ") as MienTru ON ChungTu{0}.MaCT=MienTru.MaCT) LEFT JOIN HoaDon{0} ON ChungTu{0}.MaSo=HoaDon{0}.MaSo) LEFT JOIN CTCongcudungcu ON ChungTu{0}.MaCT = CTCongcudungcu.MaCTKT) LEFT JOIN HeThongTK ON ChungTu{0}.MaTKNo = HeThongTK.MaSo) LEFT JOIN HeThongTK AS HeThongTK_1 ON ChungTu{0}.MaTKCo = HeThongTK_1.MaSo WHERE (1=1)", sh, (IIf(pNN = 1, String.Empty, String.Empty)))
        Else
            sql = String.Format("SELECT DISTINCT ChungTu{0}.MaCT, ChungTu{0}.SoHieu, NgayCT, NgayGS, ChungTu{0}.DienGiai{1} AS DG,TPS,[User_ID],ChungTu{0}.MaLoai FROM ((((ChungTu{0} INNER JOIN (" & mientru & ") as MienTru ON ChungTu{0}.MaCT=MienTru.MaCT) LEFT JOIN HoaDon{0} ON ChungTu{0}.MaSo=HoaDon{0}.MaSo) LEFT JOIN CTTaiSan ON ChungTu{0}.MaCT = CTTaiSan.MaCTKT) LEFT JOIN HeThongTK ON ChungTu{0}.MaTKNo = HeThongTK.MaSo) LEFT JOIN HeThongTK AS HeThongTK_1 ON ChungTu{0}.MaTKCo = HeThongTK_1.MaSo WHERE (1=1)", sh, (IIf(pNN = 1, String.Empty, String.Empty)))
        End If
        ...
        If Not (loaict.Length = 0) Then
            ...
            If (ChkLoai(13).CheckState = CheckState.Checked Or ChkLoai(1).CheckState = CheckState.Checked Or ChkLoai(2).CheckState = CheckState.Checked Or ChkLoai(8).CheckState = CheckState.Checked) And CboN(0).SelectedIndex > 0 Then sql = sql & " AND (ChungTu" & sh & ".MaKho=" & ConvertToStrSafe(CboN(0).SelectedValue) & ")"
            If (ChkLoai(13).CheckState = CheckState.Checked Or ChkLoai(1).CheckState = CheckState.Checked Or ChkLoai(2).CheckState = CheckState.Checked Or ChkLoai(8).CheckState = CheckState.Checked) And CboN(1).SelectedIndex > 0 Then sql = sql & " AND (ChungTu" & sh & ".MaNguon=" & ConvertToStrSafe(CboN(1).SelectedValue) & ")"
            If pProcessMode = 1 Then sql = String.Format("{0} AND XuLy<2 ", sql)
    ...
    'End Sub
    Private Sub KiemtraUser()
        '_ChkLoai_3.Enabled = (ConvertToDblSafe(frmMain.Tag) Mod 10 >= 1)
        '_ChkLoai_0.Enabled = (ConvertToDblSafe(frmMain.Tag) Mod 10 >= 1)
        'If Not (ConvertToDblSafe(frmMain.Tag) Mod 10 >= 1) Then
        '    _ChkLoai_0.CheckState = CheckState.Unchecked
        '    _ChkLoai_1.Enabled = (ConvertToDblSafe(frmMain.Tag) Mod 100 >= 10)
        '    _ChkLoai_2.Enabled = (ConvertToDblSafe(frmMain.Tag) Mod 1000 >= 100)
        '    _ChkLoai_9.Enabled = (ConvertToDblSafe(frmMain.Tag) Mod 10000 >= 1000)
        '    _ChkLoai_10.Enabled = (ConvertToDblSafe(frmMain.Tag) Mod 10000 >= 1000)
        '    _ChkLoai_11.Enabled = (ConvertToDblSafe(frmMain.Tag) Mod 10000 >= 1000)
        '    _ChkLoai_12.Enabled = (ConvertToDblSafe(frmMain.Tag) Mod 10000 >= 1000)
        '    _ChkLoai_13.Enabled = (ConvertToDblSafe(frmMain.Tag) Mod 10000 >= 1000)
        '    _ChkLoai_7.Enabled = (ConvertToDblSafe(frmMain.Tag) Mod 100000 >= 10000)
        '    _ChkLoai_8.Enabled = (ConvertToDblSafe(frmMain.Tag) Mod 100000 >= 10000)
        'End If
        _ChkLoai_3.Enabled = (frmMain.Tag Mod 10 >= 1)
        If Not frmMain.Tag Mod 10 >= 1 Then
            _ChkLoai_0.Enabled = (frmMain.Tag Mod 1000 >= 100) Or (frmMain.Tag Mod 10000 >= 1000) Or (frmMain.Tag Mod 10000000 >= 1000000)

            _ChkLoai_1.Enabled = (frmMain.Tag Mod 100 >= 10) Or (frmMain.Tag Mod 100000000 >= 10000000)
            _ChkLoai_2.Enabled = (frmMain.Tag Mod 100 >= 10) Or (frmMain.Tag Mod 100000000 >= 10000000)
            _ChkLoai_8.Enabled = (frmMain.Tag Mod 100 >= 10)

            _ChkLoai_8.Enabled = (frmMain.Tag Mod 100000 >= 10000)
            _ChkLoai_10.Enabled = (frmMain.Tag Mod 100000 >= 10000)
            _ChkLoai_11.Enabled = (frmMain.Tag Mod 100000 >= 10000)
            _ChkLoai_12.Enabled = (frmMain.Tag Mod 100000 >= 10000)

            _ChkLoai_13.Enabled = (frmMain.Tag Mod 1000000 >= 100000)
            _ChkLoai_14.Enabled = (frmMain.Tag Mod 1000000 >= 100000)
        End If
    End Sub
    ''' <summary>
    ''' 
    ...
    Private Sub DSCTu(ByRef dest As Integer)
        ...
        With GrdChungtu
            ...
            Next
        End With
        Dim sql As String = String.Format("SELECT -1 AS LoaiCT,MaCT,ChungTu.SoHieu AS SHCT,NgayCT,DienGiai{0},Sum(SoPS) AS PS,Sum(SoPS2No) AS Luong,HethongTK.SoHieu AS SHTK,HethongTK.Ten AS TenTK, CASE WHEN (TK_ID={1}) THEN Vattu.SoHieu ELSE '' END AS SHVT, TenVattu AS TenVT, CASE WHEN (TK_ID={2} OR TK_ID={3}) THEN KhachHang.SoHieu ELSE '' END AS SHKH,KhachHang.Ten AS TenKH FROM ((({4}) INNER JOIN BaoCaoCP2 ON ChungTu.MaCT=BaoCaoCP2.MaSo) LEFT JOIN Vattu ON ChungTu.MaVattu=Vattu.MaSo) LEFT JOIN KhachHang ON ChungTu.MaKH=KhachHang.MaSo GROUP BY HeThongTK.Ten,vattu.TenVattu,KhachHang.Ten,MaCT,ChungTu.SoHieu,NgayCT,DienGiai,HethongTK.SoHieu, CASE WHEN(TK_ID={1}) THEN Vattu.SoHieu ELSE '' END, CASE WHEN (TK_ID={2} OR TK_ID={3}) THEN KhachHang.SoHieu ELSE '' END UNION SELECT 1 AS LoaiCT,MaCT,ChungTu.SoHieu AS SHCT,NgayCT,DienGiai,Sum(SoPS) AS PS,Sum(SoPS2Co) AS Luong,HethongTK.SoHieu AS SHTK,HethongTK.Ten AS TenTK, CASE WHEN (TK_ID={5}) THEN Vattu.SoHieu ELSE '' END AS SHVT, TenVattu AS TenVT, CASE WHEN (TK_ID={2} OR TK_ID={3}) THEN KhachHang.SoHieu ELSE '' END AS SHKH,KhachHang.Ten AS TenKH FROM ((({6}) INNER JOIN BaoCaoCP2 ON ChungTu.MaCT=BaoCaoCP2.MaSo) LEFT JOIN Vattu ON ChungTu.MaVattu=Vattu.MaSo) LEFT JOIN KhachHang ON ChungTu.MaKHC=KhachHang.MaSo GROUP BY HeThongTK.Ten,vattu.TenVattu,KhachHang.Ten, MaCT,ChungTu.SoHieu,NgayCT,DienGiai,HethongTK.SoHieu,CASE WHEN (TK_ID={1}) THEN Vattu.SoHieu ELSE '' END, CASE WHEN (TK_ID={2} OR TK_ID={3}) THEN KhachHang.SoHieu ELSE '' END", _
    ...
    ''' <param name="eventArgs"></param>
    ''' <remarks></remarks>
    Private Sub FrmDsCTTonghop_Closed(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Closed
