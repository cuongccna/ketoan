...
Partial Friend Class FrmSovattuhanghoa
    ...
    ''' <param name="eventArgs"></param>
    ''' <remarks></remarks>
    Private Sub FrmSovattuhanghoa_KeyDown(ByVal eventSender As Object, ByVal eventArgs As KeyEventArgs) Handles MyBase.KeyDown
    ...
    Public Sub Command_ClickEvent(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles _Command_3.Click, _Command_2.Click, _Command_1.Click, _Command_0.Click
        ...
        Select Case Index
            Case 0, 1
                Me.Cursor = Cursors.WaitCursor
                GauGe.Value = 0
                If CboThang(0).Enabled And CboThang(1).SelectedIndex < CboThang(0).SelectedIndex Then
                ...
                Select Case baocao
                    Case 74
                        ...
                        mloai = IIf(ChkDu(3).CheckState = CheckState.Checked, CboLoai(0).SelectedValue, 0)
                        ...
                        GoTo LoiBC
                    Case 43
                        If CboKho(0).Items.Count > 0 Then
                            Baocao152(IIf(OptKho(0).Checked, CboKho(0).SelectedValue, 0), tcuoi)
                            GoTo LoiBC
                        End If
                    Case 12, 64
                        If CboKho(0).SelectedIndex < 0 Then
                        ...
                        If ChkDu(16).CheckState Then
                            FrmChitietThekho.LietKethekho(IIf(OptKho(0).Checked, CboKho(0).SelectedValue, 0), ConvertToDblSafe(txtShVT(0).Tag), IIf(OptTG(0).Checked, tdau, Ngay(1).ToOADate), IIf(OptTG(0).Checked, tcuoi, Ngay(2).ToOADate), True)
                            GoTo LoiBC
                        Else
                            If OptTG(0).Checked Then
                                If Not InTheKho2(IIf(OptKho(0).Checked, CboKho(0).SelectedValue, 0), _
                                           ...
                                           IIf(baocao = 64, 1, 0)) Then GoTo LoiBC
                            Else
                    ...
                    Case 13
                        ...
                        If Index = 0 Then
                            If InTheKho3(mkho, tdau, tcuoi) Then SetRptInfo() : InBaoCaoRPT()
                        Else
                            rs_tk = ExecSQLReturnDT("SELECT DISTINCT TonKho.MaVattu, Vattu.SoHieu, Vattu.TenVattu, Vattu.DonVi FROM" & " TonKho INNER JOIN Vattu ON TonKho.MaVattu=Vattu.MaSo WHERE TonKho.MaSoKho=" & mkho & " ORDER BY Vattu.SoHieu")
                            k = ConvertToDblSafe(FrmGetStr.GetString(Ngonngu("Nhập số 0 để in sổ chi tiết vật tư hàng hoá, số 2 để in thẻ kho", "0 to print receipt books detailing supplies and goods, 2 to be in stock"), Ngonngu("In toàn bộ", "Print all")))
                            GauGe.Maximum = 100
                            For Each rstkItem As DataRow In rs_tk.Rows
                                txtShVT(0).Text = ConvertToStrSafe(rstkItem("SoHieu"))
                                LbTenVT(0).Text = ConvertToStrSafe(rstkItem("TenVattu"))
                                txtShVT(0).Tag = ConvertToStrSafe(rstkItem("MaVattu"))
                                SetRptInfo()
                                If InTheKho2(mkho, _
                                       rstkItem("MaVattu"), _
                                       tdau, _
                                       ...
                                       "", _
                                       k, 0, 1) Then
                                    HienThongBao(String.Format("{0} - {1}", rstkItem("SoHieu"), rstkItem("TenVattu")), 1)
                                    InBaoCaoRPT()
                                    AppIdle(Pdelay * 100)
                                End If
                    ...
                    Case 14, 65
                        ...
                        Else
                            If OptTG(0).Checked Then
                                InLuyKe(GauGe, mkho, IIf(ChkDu(2).CheckState = CheckState.Unchecked, "", txtShTk(4).Text), mloai, tdau, tcuoi, True, IIf(baocao = 65, 1, 0))
                            Else
                                InLuyKeN(GauGe, mkho, IIf(ChkDu(2).CheckState = CheckState.Unchecked, "", txtShTk(4).Text), mloai, Ngay(1), Ngay(2), True, IIf(baocao = 65, 1, 0))
                            End If
                        End If
                    Case 46
                        If CboKho(0).SelectedIndex < 0 Then
                    ...
                    Case 52
                        If CboKho(0).SelectedIndex < 0 Then
                            ...
                            GoTo LoiBC
                        End If
                        InLCNoiBo(CboKho(0).SelectedValue, IIf(OptTG(0).Checked, tdau, 0), IIf(OptTG(0).Checked, tcuoi, 0), Ngay(1), Ngay(2))
                        GoTo LoiBC
                    Case 47, 48
                        If CboKho(0).SelectedIndex < 0 Then
                            ErrMsg(er_KhoHang)
                    ...
                    Case 15, 16
                        ...
                        mtk = IIf(ChkDu(2).CheckState = CheckState.Unchecked, 0, ConvertToDblSafe(txtShTk(4).Tag))
                        mloai = IIf(ChkDu(3).CheckState = CheckState.Checked, CboLoai(0).SelectedValue, 0)
                        MN = IIf(ChkDu(4).CheckState = CheckState.Checked, CboLoai(1).SelectedValue, 0)
...
LoiBC:
    ...
    Private Sub FrmSovattuhanghoa_Load(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Load
        Try
            Me.Cursor = Cursors.WaitCursor
            'set icon
    ...
    ''' <param name="eventArgs"></param>
    ''' <remarks></remarks>
    Private Sub FrmSovattuhanghoa_Closed(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Closed
        StopPrint = True
        xlapp = Nothing
        xlsheet = Nothing
    ...
    Sub Baocao152(ByRef mkho As Integer, ByRef tcuoi As Integer)
        ...
        ' shpl = shtk = kh = String.Empty
        'Recycle(String.Format("{0}KIEMKE.XLS", pCurDir))
        'On Error GoTo KetThuc
        'File.Copy(String.Format("{0}REPORTS\KIEMKE.XLS", pCurDir), String.Format("{0}KIEMKE.XLS", pCurDir))
        ...
        For Each rsItem As DataRow In rs.Rows
            j += 1
            While shtk <> ConvertToStrSafe(rsItem("shtk"))
                shtk = ConvertToStrSafe(rsItem("shtk"))
                xlsheet.Cells(7 + j, 2) = rsItem("ten")
                xlsheet.Range("A" & 7 + j, "T" & 7 + j).Font.Bold = True
                j += 1
            End While
...
KetThuc:
    ...
    Sub InLCNoiBo(ByRef mkho As Integer, ByRef tdau As Integer, ByRef tcuoi As Integer, ByRef ndau As Date, ByRef ncuoi As Date)
        ...
        '            GauGe.Maximum = 5
        '            Recycle(String.Format("{0}LCNoiBo.XLS", pCurDir))
        '            On Error GoTo KetThuc
        '            File.Copy(String.Format("{0}REPORTS\LCNoiBo.XLS", pCurDir), String.Format("{0}LCNoiBo.XLS", pCurDir))
        '            xlapp = New Excel.Application()
        '            xlapp.Workbooks.Open(String.Format("{0}LCNoiBo.XLS", pCurDir))
        '            xlsheet = xlapp.Worksheets(1)
        '            On Error GoTo 0
        ...
        '                    xlsheet.Cells(8 + i, 5 + 2 * j) = Format((GetSelectValue(String.Format("SELECT SUM(SoPS2Co) AS F1,Sum(SoPS) AS F2 FROM ChungTu WHERE MaLoai=4 AND MaKho={0} AND {1} AND MaNguon={2} AND MaVattu={3}", _
        '                                                                                 mkho, _
        '                                                                                 wsql, _
        ...
        '                Next
        '                xlsheet.Range("E" & 8 + i).Formula = String.Format("={0}" = sh1) 'Excel Fomular
        '                xlsheet.Range("F" & 8 + i).Formula = String.Format("={0}" = st) 'Excel Fomular
        '                sh1 = ""
        ...
        '            GauGe.Value = 4
        '            For i = 1 To sokho
        '                xlsheet.Range(XLSCol(6 + 2 * i) & ConvertToStrSafe(sovt + 9)).Formula = String.Format("=SUM({0}9:{0}{1})" = XLSCol(6 + 2 * i), ConvertToStrSafe(sovt + 8)) 'Excel Fomular
        '                xlsheet.Cells(7, 5 + 2 * i) = tenkho(ConvertToDblSafe((xlsheet.Cells(7, 5 + 2 * i))))
        '            Next
        '            xlsheet.Range("F" & sovt + 9).Formula = String.Format("=SUM(F9:F{0})" = (sovt + 8)) 'Excel Fomular
        '            With xlsheet.Range("A7", XLSCol(2 * sokho + 6) & ConvertToStrSafe(sovt + 9))
        '                .Borders.LineStyle = Excel.XlLineStyle.xlContinuous
        '                .Borders.Weight = Excel.XlBorderWeight.xlThin
        '                Dim tempRefParam As Object = .Borders.Color
        '                tempRefParam = Color.FromArgb(0, 0, 0)
        ...
        If Not (rs.Rows.Count = 0) Then
            ...
            For i = 1 To sovt
                ...
                sh1 = ""
                xlsheet.Cells(8 + i, 1) = ConvertToStrSafe(i)
                xlsheet.Cells(8 + i, 2) = TenVT(sh1, ConvertToDblSafe((xlsheet.Cells(8 + i, 2).value)), st)
                xlsheet.Cells(8 + i, 3) = sh1
                xlsheet.Cells(8 + i, 4) = st
            Next
            GauGe.Value = 4
            For i = 1 To sokho
                xlsheet.Range(XLSCol(6 + 2 * i) & ConvertToStrSafe(sovt + 9)).Formula = String.Format("=SUM({0}9:{0}{1})" = XLSCol(6 + 2 * i), ConvertToStrSafe(sovt + 8)) 'Excel Fomular
                xlsheet.Cells(7, 5 + 2 * i) = tenkho(ConvertToDblSafe((xlsheet.Cells(7, 5 + 2 * i).value)))
            Next
            xlsheet.Range("F" & sovt + 9).Formula = String.Format("=SUM(F9:F{0})", (sovt + 8)) 'Excel Fomular
            With xlsheet.Range("A7", XLSCol(2 * sokho + 6) & ConvertToStrSafe(sovt + 9))
                .Borders.LineStyle = Excel.XlLineStyle.xlContinuous
...
KetThuc:
    ...
    Sub BKSanLuong(ByRef mk As Integer, ByRef mloai As Integer, ByRef tdau As Integer, ByRef tcuoi As Integer, ByRef ndau As Date, ByRef ncuoi As Date, ByRef shtk As String)
        Dim rs As DataTable
        Dim pl3, pl1, pl2, wsql As String
        ...
        If mk > 0 Then
            ...
            Next
            If sokho > 0 Then xlsheet.Range("M7", String.Format("{0}7", XLSCol(12 + 2 * sokho))).MergeCells = True
            If sokho > 1 Then
            ...
            End If
        End If
        rs = ExecSQLReturnDT("SELECT NguonNhapXuat.DienGiai, NguonNhapXuat.MaSo FROM NguonNhapXuat INNER JOIN ChungTu ON NguonNhapXuat.MaSo=ChungTu.MaNguon WHERE ChungTu.MaLoai=8 AND " & wsql & (IIf(mk > 0, " AND MaKho=" & mk, String.Empty)) & " GROUP BY NguonNhapXuat.DienGiai, NguonNhapXuat.MaSo")
        For Each rsItem As DataRow In rs.Rows
            somn += 1
        ...
        For Each rsItem As DataRow In rs.Rows
            ...
            If mk > 0 Then
                For i As Integer = 1 To sokho
                    luong = XuatTheoKho(mk, mkho(i - 1), ConvertToDblSafe(rsItem("mvt")), ndau, ncuoi, tien)
            ...
            For i As Integer = 1 To somn
                luong = XuatBan(shtk, mk, MN(i - 1), ConvertToDblSafe(rsItem("mvt")), ndau, ncuoi, tien)
        ...
        If r > 1 Then
            ...
            For i As Integer = 6 To 12 + 2 * sokho + 2 * somn
                xlsheet.Range(XLSCol(i) & ConvertToStrSafe(r + 1)).Formula = String.Format("=Sum({0}10:{0}{1})" = XLSCol(i), ConvertToStrSafe(r)) 'Excel Fomular
            Next
            For i As Integer = 5 To 8 + sokho + somn
                xlsheet2.Range(XLSCol(i) & ConvertToStrSafe(r + 1)).Formula = String.Format("=Sum({0}10:{0}{1})", XLSCol(i), ConvertToStrSafe(r)) 'Excel Fomular
                xlsheet3.Range(XLSCol(i) & ConvertToStrSafe(r + 1)).Formula = String.Format("=Sum({0}10:{0}{1})", XLSCol(i), ConvertToStrSafe(r)) 'Excel Fomular
            Next
        End If
        With xlsheet.Range("A7", XLSCol(12 + 2 * sokho + 2 * somn) & ConvertToStrSafe(r + 1))
            .Borders.LineStyle = Excel.XlLineStyle.xlContinuous
            .Borders.Weight = Excel.XlBorderWeight.xlThin
            Dim tempRefParam As Object = .Borders.Color
            tempRefParam = Color.FromArgb(0, 0, 0)
            ...
            tempRefParam3 = Excel.XlVAlign.xlVAlignCenter
        End With
        With xlsheet2.Range("A7", XLSCol(8 + sokho + somn) & ConvertToStrSafe(r + 1))
            .Borders.LineStyle = Excel.XlLineStyle.xlContinuous
            .Borders.Weight = Excel.XlBorderWeight.xlThin
        ...
        End With
        With xlsheet3.Range("A7", XLSCol(8 + sokho + somn) & ConvertToStrSafe(r + 1))
            .Borders.LineStyle = Excel.XlLineStyle.xlContinuous
            .Borders.Weight = Excel.XlBorderWeight.xlThin
        ...
        End With
        xlsheet.Range("A1", XLSCol(12 + 2 * sokho + 2 * somn) & ConvertToStrSafe(r + 1)).Copy()
        xlapp.Worksheets.Add(xlapp.Sheets(4), , 1)
        xlsheet = xlapp.Worksheets(4)
        xlsheet.Range("A1", XLSCol(12 + 2 * sokho + 2 * somn) & ConvertToStrSafe(r + 1)).PasteSpecial()
        xlsheet.Cells(4, 1) = "Phiếu xuất thành phẩm"
        xlsheet.Range("E1", "H" & r + 1).Delete()
        xlsheet.Range("K1", "L" & r + 1).Delete()
        xlapp.Workbooks(1).Save()
        xlapp.Workbooks.Close()
        xlsheet2 = Nothing
        xlsheet3 = Nothing
...
KetThuc:
    ...
    Sub InBangKeTon(ByRef tcuoi As Integer, ByRef Ngay As Date, ByRef mtk As Integer, ByRef mloai As Integer)
        ...
        '        'ExecSQLNonQuery("DELETE FROM BaoCaoCP2")
        '        'ExecSQLNonQuery("INSERT INTO BaoCaoCP2 (MaSo,SoHieu,MaCha,BC_ID,Kq1,Kq2) SELECT (ChungTu.MaSo),ConvertToStrSafe((ChungTu.MaSo)),MaKho,MaVattu,Sum(SoPS2Co),Sum(SoPS) FROM (" & ChungTu2TKNC(1) & ") INNER JOIN BaoCaoCP ON ChungTu.MaKho=BaoCaoCP.MaCha AND ChungTu.MaVattu=BaoCaoCP.BC_ID " & " WHERE (MaLoai=2 OR MaLoai=4) AND TK_ID=" & TKVT_ID & " AND " & wsql & (iif(mtk > 0, " AND MaTKCo=" & mtk, String.Empty)) & " GROUP BY MaKho,MaVattu")
        ...
        '            On Error GoTo KetThuc
        '            File.Copy(pCurDir & "REPORTS\LCNoiBo.XLS", pCurDir & "LCNoiBo.XLS")
        '            xlapp = New Excel.Application()
        ...
        '                    xlsheet.Cells(8 + i, 6 + 2 * j) = cstr(T)
        '                    sh1 = sh1 & "+" & XLSCol(5 + 2 * j) & ConvertToStrSafe(8 + i)
        '                    st = st & "+" & XLSCol(6 + 2 * j) & ConvertToStrSafe(8 + i)
        '                Next
        '                xlsheet.Range("E" & 8 + i).Formula = String.Format("={0}" = sh1) 'Excel Fomular
        '                xlsheet.Range("F" & 8 + i).Formula = String.Format("={0}" = st) 'Excel Fomular
        '                sh1 = ""
        ...
        ExecSQLNonQuery("DELETE FROM BaoCaoCP")
        GauGe.Maximum = 3
        Dim str As String = String.Format("INSERT INTO BaoCaoCP (MaSo,SoHieu,MaCha,BC_ID,Kq1,Kq2,Ten) SELECT min(TonKho.MaSo),min(TonKho.MaSo),MaSoKho,MaVattu,Sum(Luong_0),Sum(Tien_0),Vattu.SoHieu FROM (TonKho INNER JOIN Vattu ON TonKho.MaVattu=Vattu.MaSo) INNER JOIN PhanLoaiVattu ON Vattu.MaPhanLoai=PhanLoaiVattu.MaSo WHERE 1=1 {0}{1} GROUP BY MaSoKho,Vattu.SoHieu,MaVattu", (IIf(mtk > 0, " AND MaTaiKhoan=" & mtk, String.Empty)), (IIf(mloai > 0, " AND PhanLoaiVattu.SoHieu LIKE '" & MaSo2SoHieu(mloai, "PhanLoaiVattu") & "%'", String.Empty)))
        ExecSQLNonQuery(str)
        ' Tinh dau ky
        ExecSQLNonQuery("DELETE FROM BaoCaoCP2")
        str = String.Format("INSERT INTO BaoCaoCP2 (MaSo,SoHieu,MaCha,BC_ID,Kq1,Kq2) SELECT max(ChungTu.MaSo),max(ChungTu.MaSo), CASE WHEN MaLoai=1 THEN MaKho ELSE MaNguon END,MaVattu,Sum(SoPS2No),Sum(SoPS) FROM ({0}) INNER JOIN BaoCaoCP ON ChungTu.MaVattu=BaoCaoCP.BC_ID AND ((ChungTu.MaKho=BaoCaoCP.MaCha AND MaLoai=1) OR (ChungTu.MaNguon=BaoCaoCP.MaCha AND MaLoai=4))  WHERE (MaLoai=1 OR MaLoai=4) AND TK_ID={1} AND {2}{3} GROUP BY CASE WHEN (MaLoai=1) THEN MaKho ELSE MaNguon END ,MaVattu", ChungTu2TKNC(-1), TKVT_ID, wsql, (IIf(mtk > 0, " AND MaTKNo=" & mtk, String.Empty)))
        ExecSQLNonQuery(str)
        ExecSQLNonQuery("UPDATE BaoCaoCP SET BaoCaoCP.Kq1=BaoCaoCP.Kq1+BaoCaoCP2.Kq1,BaoCaoCP.Kq2=BaoCaoCP.Kq2+BaoCaoCP2.Kq2 from BaoCaoCP2 where BaoCaoCP.MaCha=BaoCaoCP2.MaCha AND BaoCaoCP.BC_ID=BaoCaoCP2.BC_ID")
        ExecSQLNonQuery("DELETE FROM BaoCaoCP2")
        Dim str1 As String = String.Format("INSERT INTO BaoCaoCP2 (MaSo,SoHieu,MaCha,BC_ID,Kq1,Kq2) SELECT min(ChungTu.MaSo),min(ChungTu.MaSo),MaKho,MaVattu,Sum(SoPS2Co),Sum(SoPS) FROM (" & ChungTu2TKNC(1) & ") INNER JOIN BaoCaoCP ON ChungTu.MaKho=BaoCaoCP.MaCha AND ChungTu.MaVattu=BaoCaoCP.BC_ID " & " WHERE (MaLoai=2 OR MaLoai=4) AND TK_ID=" & TKVT_ID & " AND " & wsql & (IIf(mtk > 0, " AND MaTKCo=" & mtk, String.Empty)) & " GROUP BY MaKho,MaVattu")
        ExecSQLNonQuery(str1)

        ExecSQLNonQuery("UPDATE BaoCaoCP SET BaoCaoCP.Kq1=BaoCaoCP.Kq1-BaoCaoCP2.Kq1,BaoCaoCP.Kq2=BaoCaoCP.Kq2-BaoCaoCP2.Kq2 from BaoCaoCP2 where BaoCaoCP.MaCha=BaoCaoCP2.MaCha AND BaoCaoCP.BC_ID=BaoCaoCP2.BC_ID")
        ExecSQLNonQuery("DELETE FROM BaoCaoCP WHERE Kq1=0 AND Kq2=0")
        Dim sql As String = "SELECT DISTINCT MaCha FROM BaoCaoCP GROUP BY MaCha order by macha"
        Dim i As Integer = 0
        Dim rs As DataTable = ExecSQLReturnDT(sql)
        If Not (rs.Rows.Count = 0) Then
            GauGe.Maximum = 5
            Recycle(pCurDir & "\temp\LCNoiBo.XLS")
            On Error GoTo KetThuc
            File.Copy(pCurDir & "\REPORTS\LCNoiBo.XLS", pCurDir & "\temp\LCNoiBo.XLS")
            xlapp = New Excel.Application()
            xlapp.Workbooks.Open(pCurDir & "\temp\LCNoiBo.XLS")
            xlsheet = xlapp.Worksheets(1)
            On Error GoTo 0
            xlsheet.Cells(1, 1) = pTenCty
            xlsheet.Cells(2, 1) = pTenCn
            ...
            For i = 1 To sovt
                ...
                st = ""
                For j As Integer = 1 To sokho
                    Dim str10 As String = "SELECT Kq1 AS F1,Kq2 AS F2 FROM BaoCaoCP WHERE MaCha=" & xlsheet.Cells(7, 5 + 2 * j).value & " and BC_ID=" & xlsheet.Cells(8 + i, 2).value
                    xlsheet.Cells(8 + i, 5 + 2 * j) = Format(ConvertToDblSafe(GetSelectValue(str10, T)), Mask_2)
                    xlsheet.Cells(8 + i, 6 + 2 * j) = CStr(T)
                    sh1 = sh1 & "+" & XLSCol(5 + 2 * j) & ConvertToStrSafe(8 + i)
                    st = st & "+" & XLSCol(6 + 2 * j) & ConvertToStrSafe(8 + i)
                Next
...
KetThuc:
    ...
    Sub InLuyKeX(ByRef mkho As Integer, ByRef mtk As Integer, ByRef mloai As Integer, ByRef tdau As Integer, ByRef tcuoi As Integer)
        ...
        Dim j As Integer
        Recycle(pCurDir & "\temp\LUYKE.XLS")
        On Error GoTo KetThuc
        File.Copy(pCurDir & "\REPORTS\LUYKE.XLS", pCurDir & "\temp\LUYKE.XLS")
        xlapp = New Excel.Application()
        xlapp.Workbooks.Open(pCurDir & "\temp\LUYKE.XLS")
        xlsheet = xlapp.Worksheets(1)
        On Error GoTo 0
        xlsheet.Cells(1, 1) = pTenCty
        xlsheet.Cells(2, 1) = pTenCn
        If mkho > 0 Then xlsheet.Cells(3, 1) = CboKho(0).Text
        xlsheet.Cells(3, 3) = ThoiGianTheoThang(tdau, tcuoi)
        GauGe.Maximum = 3
        ...
        sql = "SELECT DISTINCT HeThongTK.MaSo AS MSTK, HeThongTK.SoHieu, HeThongTK.Ten, PhanLoaiVattu.MaSo," & " PhanLoaiVattu.SoHieu, PhanLoaiVattu.TenPhanLoai, Vattu.MaSo AS MSVT, Vattu.SoHieu AS SHVT, Vattu.TenVattu, Vattu.DonVi, Vattu.DonVi2, Vattu.Dvt2, Sum(TonKho.Tien_" & st & ") AS DkTien," & " Sum(TonKho.Luong_" & st & ") AS DkLuong, Sum(" & sqltn.ToString() & ") AS TienNhap, Sum(" & sqlln.ToString() & ") AS LuongNhap, Sum(" & sqltx.ToString() & ") AS TienXuat," & " Sum(" & sqllx.ToString() & ") AS LuongXuat, Sum(TonKho.Luong_" & ConvertToStrSafe(CThangDB2(tcuoi)) & ") AS CkLuong, Sum(TonKho.Tien_" & ConvertToStrSafe(CThangDB2(tcuoi)) & ") AS CkTien" & " FROM HeThongTK RIGHT JOIN ((PhanLoaiVattu RIGHT JOIN Vattu ON PhanLoaiVattu.MaSo = Vattu.MaPhanLoai) RIGHT JOIN TonKho ON Vattu.MaSo = TonKho.MaVatTu) ON HeThongTK.MaSo = TonKho.MaTaiKhoan" & " GROUP BY HeThongTK.MaSo, HeThongTK.SoHieu, HeThongTK.Ten, PhanLoaiVattu.MaSo, PhanLoaiVattu.SoHieu, PhanLoaiVattu.TenPhanLoai, Vattu.MaSo, Vattu.SoHieu, Vattu.TenVattu, Vattu.DonVi2,Vattu.DonVi,Vattu.Dvt2 " & (IIf(mkho > 0, ", TonKho.MaSoKho", String.Empty)) & " Having (Sum(TonKho.Luong_" & ConvertToStrSafe(CThangDB2(ThangTruoc(tdau))) & ")<>0 OR Sum(TonKho.Tien_" & ConvertToStrSafe(CThangDB2(ThangTruoc(tdau))) & ")<>0 OR SUM(" & sqlln.ToString() & ")<>0 OR SUM(" & sqltn.ToString() & ")<>0 OR SUM(" & sqllx.ToString() & ")<>0 OR SUM(" & sqltx.ToString() & ")<>0)" & (IIf(mkho <> 0, " AND TonKho.MaSoKho = " & mkho, String.Empty)) & (IIf(mtk <> 0, " AND HethongTK.MaSo = " & mtk, String.Empty)) & (IIf(mloai <> 0, " AND PhanLoaiVattu.SoHieu LIKE '" & MaSo2SoHieu(mloai, "PhanLoaiVattu") & "%'", String.Empty)) & " ORDER BY HeThongTK.SoHieu, PhanLoaiVattu.SoHieu, Vattu.SoHieu"
        i = 0
        rs = ExecSQLReturnDT(sql)
        For Each rsItem As DataRow In rs.Rows
            i += 1
            xlsheet.Range(String.Format("{0}:{1}", ConvertToStrSafe(i + 6 + 1), ConvertToStrSafe(i + 6 + 1))).EntireRow.Insert()
            xlsheet.Cells(i + 6, 1) = i
            xlsheet.Cells(i + 6, 2) = ConvertToStrSafe(rsItem("TenVattu"))
            xlsheet.Cells(i + 6, 3) = ConvertToStrSafe(rsItem("shvt"))
            xlsheet.Cells(i + 6, 4) = ConvertToStrSafe(rsItem("DonVi"))
            If ConvertToDblSafe(rsItem("dkluong")) <> 0 Then xlsheet.Cells(i + 6, 5) = ConvertToStrSafe(rsItem("dkluong"))
            If ConvertToDblSafe(rsItem("dktien")) <> 0 Then xlsheet.Cells(i + 6, 6) = ConvertToStrSafe(rsItem("dktien"))
            If ConvertToDblSafe(rsItem("luongnhap")) <> 0 Then xlsheet.Cells(i + 6, 7) = ConvertToDblSafe(rsItem("luongnhap"))
            If ConvertToDblSafe(rsItem("tiennhap")) <> 0 Then xlsheet.Cells(i + 6, 8) = ConvertToDblSafe(rsItem("tiennhap"))
            If ConvertToDblSafe(rsItem("luongxuat")) <> 0 Then xlsheet.Cells(i + 6, 9) = ConvertToDblSafe(rsItem("luongxuat"))
            If ConvertToDblSafe(rsItem("tienxuat")) <> 0 Then xlsheet.Cells(i + 6, 10) = ConvertToDblSafe(rsItem("tienxuat"))
            If ConvertToDblSafe(rsItem("ckluong")) <> 0 Then xlsheet.Cells(i + 6, 11) = ConvertToDblSafe(rsItem("ckluong"))
            If ConvertToDblSafe(rsItem("CKTien")) <> 0 Then xlsheet.Cells(i + 6, 13) = ConvertToDblSafe(rsItem("CKTien"))
            ...
            lx += l1
            tx += T
            If lx <> 0 Then xlsheet.Cells(i + 6, 7) = lx
            If tx <> 0 Then xlsheet.Cells(i + 6, 8) = tx
            NhapTheoNguon(tdau, _
                tcuoi, _
                ConvertToDblSafe(rsItem("MSVT")), _
                ConvertToDblSafe(rsItem("MSTK")), _
            ...
            lx += l1
            tx += T
            If lx <> 0 Then xlsheet.Cells(i + 6, 9) = lx
            If tx <> 0 Then xlsheet.Cells(i + 6, 10) = tx
            ...
            lx += l1
            tx += T
            If lx <> 0 Then xlsheet.Cells(i + 6, 15) = lx
            If tx <> 0 Then xlsheet.Cells(i + 6, 16) = tx
            XuatTheoNguon(tdau, _
                tcuoi, _
                ConvertToDblSafe(rsItem("MSVT")), _
                ConvertToDblSafe(rsItem("MSTK")), _
                ...
                tx, _
                mkho)
            If lx <> 0 Then xlsheet.Cells(i + 6, 17) = lx
            If tx <> 0 Then xlsheet.Cells(i + 6, 18) = tx
            XuatTheoNguon(tdau, _
                tcuoi, _
                ConvertToDblSafe(rsItem("MSVT")), _
                ConvertToDblSafe(rsItem("MSTK")), _
                ...
                tx, _
                mkho)
            If lx <> 0 Then xlsheet.Cells(i + 6, 19) = lx
            If tx <> 0 Then xlsheet.Cells(i + 6, 20) = tx
            xlsheet.Range("K" & i + 6).Formula = "=M" & i + 6 & "-G" & ConvertToStrSafe(i + 6) & "-I" & ConvertToStrSafe(i + 6) 'Excel Fomular
            xlsheet.Range("L" & i + 6).Formula = "=N" & i + 6 & "-H" & ConvertToStrSafe(i + 6) & "-J" & ConvertToStrSafe(i + 6) 'Excel Fomular
            xlsheet.Range("U" & i + 6).Formula = "=W" & i + 6 & "-O" & ConvertToStrSafe(i + 6) & "-Q" & ConvertToStrSafe(i + 6) & "-S" & ConvertToStrSafe(i + 6) 'Excel Fomular
            xlsheet.Range("V" & i + 6).Formula = "=X" & i + 6 & "-P" & ConvertToStrSafe(i + 6) & "-R" & ConvertToStrSafe(i + 6) & "-T" & ConvertToStrSafe(i + 6) 'Excel Fomular
        Next
        GauGe.Value = 2
        xlsheet.Cells(i + 7, 2) = ("Tổng cộng")
        xlsheet.Range("F" & i + 7).Formula = "=SUM(F7:F" & i + 6 & ")" 'Excel Fomular
        xlsheet.Range("G" & i + 7).Formula = "=SUM(G7:G" & i + 6 & ")" 'Excel Fomular
        xlsheet.Range("H" & i + 7).Formula = "=SUM(H7:H" & i + 6 & ")" 'Excel Fomular
        xlsheet.Range("I" & i + 7).Formula = "=SUM(I7:I" & i + 6 & ")" 'Excel Fomular
        xlsheet.Range("J" & i + 7).Formula = "=SUM(J7:J" & i + 6 & ")" 'Excel Fomular
        xlsheet.Range("K" & i + 7).Formula = "=SUM(K7:K" & i + 6 & ")" 'Excel Fomular
        xlsheet.Range("L" & i + 7).Formula = "=SUM(L7:L" & i + 6 & ")" 'Excel Fomular
        xlsheet.Range("M" & i + 7).Formula = "=SUM(M7:M" & i + 6 & ")" 'Excel Fomular
        'xlsheet.Range("N" & i + 7).Formula = "=SUM(N10:N" & i + 6 & ")" 'Excel Fomular
        'xlsheet.Range("O" & i + 7).Formula = "=SUM(O10:O" & i + 6 & ")" 'Excel Fomular
        'xlsheet.Range("P" & i + 7).Formula = "=SUM(P10:P" & i + 6 & ")" 'Excel Fomular
        'xlsheet.Range("Q" & i + 7).Formula = "=SUM(Q10:Q" & i + 6 & ")" 'Excel Fomular
        'xlsheet.Range("R" & i + 7).Formula = "=SUM(R10:R" & i + 6 & ")" 'Excel Fomular
        'xlsheet.Range("S" & i + 7).Formula = "=SUM(S10:S" & i + 6 & ")" 'Excel Fomular
        'xlsheet.Range("T" & i + 7).Formula = "=SUM(T10:T" & i + 6 & ")" 'Excel Fomular
        'xlsheet.Range("U" & i + 7).Formula = "=SUM(U10:U" & i + 6 & ")" 'Excel Fomular
        'xlsheet.Range("V" & i + 7).Formula = "=SUM(V10:V" & i + 6 & ")" 'Excel Fomular
        'xlsheet.Range("W" & i + 7).Formula = "=SUM(W10:W" & i + 6 & ")" 'Excel Fomular
        'xlsheet.Range("X" & i + 7).Formula = "=SUM(X10:X" & i + 6 & ")" 'Excel Fomular
        'xlsheet.Range("Y" & i + 7).Formula = "=SUM(Y10:Y" & i + 6 & ")" 'Excel Fomular
        'xlsheet.Range("Z" & i + 7).Formula = "=SUM(Z10:Z" & i + 6 & ")" 'Excel Fomular
        With xlsheet.Range("A7", "Z" & i + 7)
            .Borders.LineStyle = Excel.XlLineStyle.xlContinuous
            .Borders.Weight = Excel.XlBorderWeight.xlThin
            Dim tempRefParam As Object = .Borders.Color
            tempRefParam = Color.FromArgb(0, 0, 0)
...
KetThuc:
    ...
    Sub XuatExcelLuyKe(ByRef GauGe As ProgressBar, ByRef mkho As Integer, ByRef mloai As Integer, ByRef tdau As Integer, ByRef tcuoi As Integer, Optional ByRef ten As String = "")
        ...
        XDTyLeQD(0)
        If mkho > 0 Then
            sql = "SELECT DISTINCT hethongtk.sohieu as [hethongtk.sohieu], Vattu.TenVattu, Vattu.DonVi, Vattu.SoHieu, Sum(TonKho.Luong_" & CThangDB2(ThangTruoc(tdau)) & ") AS DkLuong," & " Sum(TonKho.Tien_" & CThangDB2(ThangTruoc(tdau)) & ") AS DkTien, Sum(" & sqlln.ToString() & ") AS LuongNhap, Sum(" & sqltn.ToString() & ") AS TienNhap," & " Sum(" & sqllx.ToString() & ") AS LuongXuat, Sum(" & sqltx.ToString() & ") AS TienXuat, Sum(TonKho.Luong_" & st & ") AS CkLuong" & (IIf(pGiaUSD > 0, ", Sum(TonKho.USDTien_" & CThangDB2(ThangTruoc(tdau)) & ") AS DkUSD, Sum(" & sqltn2.ToString() & ") AS TienNhapUSD, Sum(" & sqltx2.ToString() & ") AS TienXuatUSD, Sum(TonKho.USDTien_" & st & ") AS CkUSD", String.Empty)) & ", HeThongTK.SoHieu AS SHTK, HeThongTK.Ten  AS TenTK, Sum(TonKho.Tien_" & st & ") AS CkTien" & " FROM HeThongTK RIGHT JOIN ((PhanLoaiVattu RIGHT JOIN Vattu ON PhanLoaiVattu.MaSo = Vattu.MaPhanLoai) RIGHT JOIN TonKho ON Vattu.MaSo = TonKho.MaVatTu) ON HeThongTK.MaSo = TonKho.MaTaiKhoan" & " GROUP BY hethongtk.sohieu,Vattu.SoHieu, Vattu.TenVattu, Vattu.DonVi, HethongTK.SoHieu, HethongTK.Ten" & (IIf(mkho > 0, ", TonKho.MaSoKho", String.Empty)) & (IIf(mloai > 0, ", PhanLoaiVattu.SoHieu", String.Empty)) & " Having (Sum(TonKho.Luong_" & st & ")<>0 OR Sum(TonKho.Tien_" & st & ")<>0 OR SUM(" & sqlln.ToString() & ")<>0 OR SUM(" & sqltn.ToString() & ")<>0 OR SUM(" & sqllx.ToString() & ")<>0 OR SUM(" & sqltx.ToString() & ")<>0" & (IIf(pGiaUSD > 0, " OR Sum(TonKho.USDTien_" & st & ")<>0 OR SUM(" & sqltn2.ToString() & ")<>0 OR SUM(" & sqltx2.ToString() & ")<>0)", ")")) & (IIf(mkho <> 0, " AND TonKho.MaSoKho = " & mkho, String.Empty)) & (IIf(mloai <> 0, " AND PhanLoaiVattu.SoHieu LIKE '" & MaSo2SoHieu(mloai, "PhanLoaiVattu") & "%'", String.Empty)) & (IIf(ten <> "", " and vattu.tenvattu like'%" & ten.ToUpper() & "%'", String.Empty))
        Else
            ...
            Exit Sub
        End If
        OpenBook(pCurDir & "\RePorts\Luyke.xls")
        ...
        End Select
        Try
            Kill(String.Format("{0}\temp\Luyke.xls", pCurDir))
        Catch
        End Try
        ...
        curSheet.Cells(2, 1) = "MST: " & frmMain._LbCty_8.Text
        curSheet.Cells(3, 1) = "Kho: " & CboKho(0).Text
        sql = "SELECT tenvattu,donvi,sohieu,dkluong,dktien,luongnhap,tiennhap,luongxuat,tienxuat,dkluong+luongnhap-luongxuat as luongton,CASE WHEN (dkluong+luongnhap-luongxuat>0) THEN (dktien+tiennhap-tienxuat)/(dkluong+luongnhap-luongxuat) ELSE 0 END,dktien+tiennhap-tienxuat as tienton,shtk,tentk,[hethongtk.sohieu] FROM (" & sql & ") as qluyke group by [hethongtk.sohieu],shtk,tentk,tenvattu,donvi,sohieu,dkluong,dktien,luongnhap,tiennhap,luongxuat,tienxuat"
        PrintSQLLuyKe(GauGe, sql, curSheet, 7, 2, tdau, tcuoi)
        curSheet.SaveAs(pCurDir & "\temp\Luyke.xls")
        CloseBook(pCurDir & "\temp\Luyke.xls")
Loi:
        CloseBook(pCurDir & "\RePorts\Luyke.xls")
        CallExcel("\temp\Luyke.xls")
    ...
    Function PrintSQLLuyKe(ByRef GauGe As ProgressBar, ByRef sql As String, ByVal curSheet As Excel.Worksheet, ByVal RowIdx As Integer, ByRef StartCol As Integer, ByRef tdau As Integer, ByRef tcuoi As Integer) As Integer
        ...
        If rs.Rows.Count > 0 Then
            ...
            While i < rs.Rows.Count
                ...
                GauGe.Value += 1
                If Dieukien <> ConvertToStrSafe(rsItem("shtk")) Then
                    If dong > 0 Then
                        If i > 0 Then
                            i -= 1
                        End If
                    ...
                    End If
                    rsItem = rs.Rows(i)
                    Dieukien = ConvertToStrSafe(rsItem("shtk"))
                    curSheet.Range(String.Format("{0}:{0}", ConvertToStrSafe(RowIdx + 1))).EntireRow.Insert()
                    dong = RowIdx
                    RowIdx += 1
                End If
                curSheet.Cells(RowIdx, 1).Value = ConvertToDblSafe(IIf(curSheet.Cells(RowIdx - 1, 1).value > 0, curSheet.Cells(RowIdx - 1, 1).Value, curSheet.Cells(RowIdx - 2, 1).Value)) + 1
                ...
                For FieldCnt As Integer = 0 To rs.Columns.Count - 4
                    curSheet.Cells(RowIdx, FieldCnt + StartCol).Value = ConvertToStrSafe(rsItem(FieldCnt).ToString)
                Next FieldCnt
                i += 1
                RowIdx += 1
            End While
            If dong > 0 Then
                ...
                curSheet.Cells(RowIdx + 2, 7).Value = "Ngày " & DateAndTime.Day(ngaybc) & " tháng " & ConvertToStrSafe(tcuoi) & " năm " & ConvertToStrSafe(pNamTC)
            End If
        End If
        Return RowIdx
    End Function
    Sub InNguonNhapXuat(ByRef GauGe As ProgressBar, ByRef mkho As Integer, ByRef mtk As Integer, ByRef mloai As Integer, ByRef MN As Integer, ByRef tdau As Integer, ByRef tcuoi As Integer, ByRef loai As Integer, ByRef dktg As Integer, ByRef ndau As Date, ByRef ncuoi As Date)
        ...
        Else
            ...
            rptFrom.SetFormulas("ThoiGian", String.Format("ThoiGian='{0}'", ThoiGianN(ndau, ncuoi, nn)))
        End If
    ...
    Sub InTHXuatKho(ByRef tdau As Integer, ByRef tcuoi As Integer, ByRef ndau As Date, ByRef ncuoi As Date, ByRef mkho As Integer, ByRef mtk As Integer, ByRef mloai As Integer, ByRef tkdu As String)
        ...
        sql = String.Format("{0} UNION SELECT DISTINCT Vattu.SoHieu,TenVattu,DonVi,-Sum(SoPS2No) AS Luong,-Sum(SoPS) AS Tien FROM ({1}) INNER JOIN Vattu ON ChungTu.MaVattu=Vattu.MaSo WHERE (ChungTu.MaLoai=1 OR (ChungTu.MaLoai=4 AND MaTKNo<>MaTKCo)) AND {2} AND HethongTK.SoHieu LIKE '{3}%'{4}{5}{6} GROUP BY Vattu.SoHieu,TenVattu,DonVi", _
                  ...
                  (IIf(mtk <> 0, String.Format(" AND MaTKNo = {0}", mtk), String.Empty)), _
                  (IIf(mloai <> 0, String.Format(" AND MaPhanLoai = {0}", mloai), String.Empty)))
        ...
        rptFrom.SetFormulas("TKDU", String.Format("'{0}{1} - {2}'", ("Reciprocal accounts: "), tkdu, LbTenTk(11).Text))
    ...
    Sub InTHXuatKhoDT(ByRef tdau As Integer, ByRef tcuoi As Integer, ByRef ndau As Date, ByRef ncuoi As Date, ByRef mkho As Integer, ByRef mtk As Integer, ByRef mloai As Integer, ByRef tkdu As Integer, ByRef xT As Integer)
        ...
        rptFrom.SetFormulas("ThoiGian", String.Format("'{0}'", (IIf(tdau > 0, ThoiGianTheoThang(tdau, tcuoi), ThoiGianN(ndau, ncuoi, nn)))))
        rptFrom.SetFormulas("TKDU", String.Format("'{0}{1} - {2}'", Ngonngu("Xuất cho đối tượng: ", "Object: "), txtCT(1).Text, LbCT(1).Text))
    ...
    Sub InLuyKeHM(ByRef GauGe As ProgressBar, ByRef mkho As Integer, ByRef mtk As Integer, ByRef mloai As Integer, ByRef ncuoi As Date, ByRef loaihm As Integer)
        ...
        ExecSQLNonQuery("DELETE FROM BaoCaoCP2")
        GauGe.Maximum = 3
        'ExecSQLNonQuery(String.Format("INSERT INTO BaoCaoCP2 (MaSo,SoHieu,MaCha,BC_ID,MK,Kq7,Kq8) SELECT TonKho.MaSo,SoHieu,MaTaiKhoan,MaVattu,MaSoKho,TonMin,TonMax FROM TonKho INNER JOIN Vattu ON TonKho.MaVattu=Vattu.MaSo WHERE {0}{1}{2}{3}", _
        ...
        '                    (IIf(mloai > 0, String.Format(" AND MaPhanLoai={0}", mloai), String.Empty))))
        Dim tam As String = String.Format("INSERT INTO BaoCaoCP2 (MaSo,SoHieu,MaCha,BC_ID,MK,Kq7,Kq8) SELECT TonKho.MaSo,SoHieu,MaTaiKhoan,MaVattu,MaSoKho,TonMin,TonMax FROM TonKho INNER JOIN Vattu ON TonKho.MaVattu=Vattu.MaSo WHERE {0}{1}{2}{3}", _
                           (IIf(loaihm < 0, " TonMin>0", " TonMax>0")), _
                           (IIf(mkho > 0, String.Format(" AND MaSoKho={0}", mkho), String.Empty)), _
                           (IIf(mtk > 0, String.Format(" AND MaTaiKhoan={0}", mtk), String.Empty)), _
                           (IIf(mloai > 0, String.Format(" AND MaPhanLoai={0}", mloai), String.Empty)))
        ...
        For Each rsItem As DataRow In rs.Rows
            ...
            If loaihm > 0 And dkl > ConvertToDblSafe(rsItem("Kq8")) Then
                ExecSQLNonQuery(String.Format("UPDATE BaoCaoCP2 SET CoCon=1 WHERE MaSo={0}", rsItem("MaSo")))
            End If
            ExecSQLNonQuery(String.Format("UPDATE BaoCaoCP2 SET Kq1={0} WHERE MaSo={1}", CStr(dkl), rsItem("MaSo")))
        Next
        ExecSQLNonQuery("DELETE FROM BaoCaoCP2 WHERE CoCon=0")
        GauGe.Value = 1
        'Dim sql As String = "SELECT PhanLoaiVattu.SoHieu, PhanLoaiVattu.TenPhanLoai, Vattu.SoHieu, Vattu.TenVattu,Vattu.TonMin,Vattu.TonMax, Kq1, Kq7,Kq8 FROM (HeThongTK RIGHT JOIN ((PhanLoaiVattu RIGHT JOIN Vattu ON PhanLoaiVattu.MaSo = Vattu.MaPhanLoai) RIGHT JOIN TonKho ON Vattu.MaSo = TonKho.MaVatTu) ON HeThongTK.MaSo = TonKho.MaTaiKhoan) INNER JOIN BaoCaoCP2 ON TonKho.MaSoKho=BaoCaoCP2.MK AND TonKho.MaTaiKhoan=BaoCaoCP2.MaCha AND TonKho.MaVattu=BaoCaoCP2.BC_ID" & " WHERE 1=1 " & (IIf(mkho <> 0, " And TonKho.MaSoKho = " & mkho, String.Empty)) & (IIf(mtk <> 0, " And HethongTK.MaSo = " & mtk, String.Empty)) & (IIf(mloai <> 0, " AND PhanLoaiVattu.SoHieu LIKE '" & MaSo2SoHieu(mloai, "PhanLoaiVattu") & "%'", String.Empty)) & " ORDER BY HeThongTK.SoHieu, PhanLoaiVattu.SoHieu, Vattu.SoHieu"
        Dim sql As String = "SELECT PhanLoaiVattu.SoHieu as [PhanLoaiVattu.SoHieu], PhanLoaiVattu.TenPhanLoai as [TenPhanLoai], Vattu.SoHieu as [Vattu.SoHieu], Vattu.TenVattu as [TenVattu],Vattu.TonMin as [TonMin],Vattu.TonMax as [TonMax],Kq1 as [Kq1], Kq7 as [Kq7],Kq8 as [Kq8]  FROM (HeThongTK RIGHT JOIN ((PhanLoaiVattu RIGHT JOIN Vattu ON PhanLoaiVattu.MaSo = Vattu.MaPhanLoai) RIGHT JOIN TonKho ON Vattu.MaSo = TonKho.MaVatTu) ON HeThongTK.MaSo = TonKho.MaTaiKhoan) INNER JOIN BaoCaoCP2 ON TonKho.MaSoKho=BaoCaoCP2.MK AND TonKho.MaTaiKhoan=BaoCaoCP2.MaCha AND TonKho.MaVattu=BaoCaoCP2.BC_ID" & " WHERE 1=1 " & (IIf(mkho <> 0, " And TonKho.MaSoKho = " & mkho, String.Empty)) & (IIf(mtk <> 0, " And HethongTK.MaSo = " & mtk, String.Empty)) & (IIf(mloai <> 0, " AND PhanLoaiVattu.SoHieu LIKE '" & MaSo2SoHieu(mloai, "PhanLoaiVattu") & "%'", String.Empty)) & " ORDER BY HeThongTK.SoHieu, PhanLoaiVattu.SoHieu, Vattu.SoHieu"
